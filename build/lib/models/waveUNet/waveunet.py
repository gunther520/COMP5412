...
class Waveunet(nn.Module):
    def __init__(self, num_inputs, num_channels, num_outputs, instruments, kernel_size, conv_type, res, separate=False, depth=1, strides=2):
        ...
        self.input_size = None  # Will be determined dynamically in first forward pass
    ...
    def forward(self, x, inst=None):
        # Store input size on first forward pass
        ...
        if self.separate:
            ...
            for idx, inst in enumerate(self.instruments):
                return  out[:, idx * self.num_outputs:(idx + 1) * self.num_outputs]
            return None