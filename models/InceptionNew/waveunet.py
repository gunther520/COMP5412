...
from models.InceptionNew.crop import centre_crop
from models.InceptionNew.resample import Resample1d
from models.InceptionNew.conv import ConvLayer
from models.InceptionNew.InceptionNewLayer import InceptionLayer

class UpsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_shortcut, n_outputs, kernel_size, stride, depth, conv_type, res ,inception_kernel=[1,3, 5, 7, 11]):
        ...
        # Replace pre_shortcut_convs in UpsamplingBlock.__init__  
        self.pre_shortcut_convs = nn.ModuleList([InceptionLayer(n_inputs, n_outputs, kernel_sizes= inception_kernel, stride=1, conv_type=conv_type)] +  
                                            [ConvLayer(n_outputs, n_outputs, kernel_size, 1, conv_type) for _ in range(depth - 1)])  
        
        # Replace post_shortcut_convs in UpsamplingBlock.__init__  
        self.post_shortcut_convs = nn.ModuleList([ConvLayer(n_outputs + n_shortcut, n_outputs, kernel_size, 1, conv_type)] +  
                                                [InceptionLayer(n_outputs, n_outputs, kernel_sizes = inception_kernel, stride=1, conv_type=conv_type) for _ in range(depth - 1)])
    ...
    def forward(self, x, shortcut):
        ...
        cropped_shortcut = centre_crop(shortcut, upsampled)
        
        # Combine features - IMPORTANT: Order matters here!
        combined = torch.cat([cropped_shortcut, upsampled], dim=1)
...
class DownsamplingBlock(nn.Module):
    def __init__(self, n_inputs, n_shortcut, n_outputs, kernel_size, stride, depth, conv_type, res , inception_kernel=[1,3,5,7,11]):
        ...
        # Replace pre_shortcut_convs in DownsamplingBlock.__init__  
        self.pre_shortcut_convs = nn.ModuleList([InceptionLayer(n_inputs, n_shortcut, kernel_sizes=inception_kernel, stride=1, conv_type=conv_type)] +   
                                            [ConvLayer(n_shortcut, n_shortcut, kernel_size, 1, conv_type) for _ in range(depth - 1)])  
        
        # Replace post_shortcut_convs in DownsamplingBlock.__init__  
...
class Waveunet(nn.Module):
    def __init__(self, num_inputs, num_channels, num_outputs, instruments, kernel_size, target_output_size, conv_type, res, separate=False, depth=1, strides=2 , inception_kernel=[1,3,5,7,11]):
        ...
        for instrument in model_list:
            ...
            for i in range(self.num_levels - 1):
                ...
                module.downsampling_blocks.append(
                    DownsamplingBlock(in_ch, num_channels[i], num_channels[i+1], kernel_size, strides, depth, conv_type, res, inception_kernel))

            for i in range(0, self.num_levels - 1):
                module.upsampling_blocks.append(
                    UpsamplingBlock(num_channels[-1-i], num_channels[-2-i], num_channels[-2-i], kernel_size, strides, depth, conv_type, res, inception_kernel))
            
            module.bottlenecks = nn.ModuleList(  
                [InceptionLayer(num_channels[-1], num_channels[-1], kernel_sizes=inception_kernel, stride=1, conv_type=conv_type) for _ in range(depth)]  
            )
    ...
    def forward(self, x, inst=None):
        ...
        #print(f"Input size: {curr_input_size}, expected: {self.input_size}")
