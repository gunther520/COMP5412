...
from models.Inception.crop import centre_crop
from models.Inception.resample import Resample1d
from models.Inception.conv import ConvLayer
from models.Inception.inception import Inception1d


class InceptionUpsamplingBlock(nn.Module):  
    def __init__(self, n_inputs, n_shortcut, n_outputs, kernel_size, stride, depth, conv_type, res):  
        super(InceptionUpsamplingBlock, self).__init__()  
        assert(stride > 1)  
          
        # CONV for UPSAMPLING (keep this as is to maintain upsampling)  
        if res == "fixed":  
        ...
        # Replace standard convs with Inception modules  
        self.pre_shortcut_convs = nn.ModuleList([Inception1d(n_inputs, n_outputs, conv_type)] +   
                                               [Inception1d(n_outputs, n_outputs, conv_type) for _ in range(depth - 1)])  
...
class InceptionDownsamplingBlock(nn.Module):  
    def __init__(self, n_inputs, n_shortcut, n_outputs, kernel_size, stride, depth, conv_type, res):  
        super(InceptionDownsamplingBlock, self).__init__()  
        assert(stride > 1)  
        ...
        # Replace standard convs with Inception modules  
        self.pre_shortcut_convs = nn.ModuleList([Inception1d(n_inputs, n_shortcut, conv_type)] +   
                                               [Inception1d(n_shortcut, n_shortcut, conv_type) for _ in range(depth - 1)])  
          
        self.post_shortcut_convs = nn.ModuleList([Inception1d(n_shortcut, n_outputs, conv_type)] +   
                                                [Inception1d(n_outputs, n_outputs, conv_type) for _ in range(depth - 1)])  
          
        # CONV with decimation (keep this as is to maintain downsampling)  
        if res == "fixed":  
            self.downconv = Resample1d(n_outputs, 15, stride)  
        else:  
            self.downconv = ConvLayer(n_outputs, n_outputs, kernel_size, stride, conv_type)  
      
    # The forward and other methods remain the same as in DownsamplingBlock
    def forward(self, x):
        ...
        for instrument in model_list:
            ...
            for i in range(self.num_levels - 1):
                ...
                module.downsampling_blocks.append(  
                    InceptionDownsamplingBlock(in_ch, num_channels[i], num_channels[i+1], kernel_size, strides, depth, conv_type, res))  
  
            for i in range(0, self.num_levels - 1):
                module.upsampling_blocks.append(  
                    InceptionUpsamplingBlock(num_channels[-1-i], num_channels[-2-i], num_channels[-2-i], kernel_size, strides, depth, conv_type, res))  
                
            module.bottlenecks = nn.ModuleList(  
                [Inception1d(num_channels[-1], num_channels[-1], conv_type) for _ in range(depth)])
